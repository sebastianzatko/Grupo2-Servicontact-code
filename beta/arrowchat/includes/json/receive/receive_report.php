	...
	{	
		...
		if (!empty($report_chatroom))
		{
			...
			foreach ($temp_array as $chatroom_history)
			{
				...
				$chat_history[] = array('id' => $chatroom_history['id'], 'n' => strip_tags($chatroom_history['username']), 'm' => $chat_message, 't' => $chatroom_history['sent'], 'a' => $avatar, 'userid' => $fetchid, 'global' => $chatroom_history['global_message'], 'mod' => $chatroom_history['is_mod'], 'admin' => $chatroom_history['is_admin'], 'reportee' => $reportee);
			}
		}
		else
		{
			...
			foreach ($temp_array as $chatroom_history)
			{
				...
				$chat_history[] = array('id' => $chatroom_history['id'], 'n' => strip_tags($username), 'm' => $chat_message, 't' => $chatroom_history['sent'], 'a' => $avatar, 'userid' => $fetchid, 'global' => 0, 'mod' => 0, 'admin' => 0, 'reportee' => $reportee);
			}
		}
