...
require_once('conexion/conectionpdo.php');
	class d_search{
			...
			$sql="SELECT
					...
					WHERE
						USUARIOS.idUSUARIO = PROFESIONALES.USUARIO_idUSUARIO AND OFICIOS.PROFESIONAL_idPROFESIONAL = PROFESIONALES.idPROFESIONAL AND OFICIOS.SERVICIOS_idSERVICIO = SERVICIOS.idSERVICIO AND(
							".$servicios."
						) AND OFICIOS.HABILITADO=1".$idusuario." AND 111.1111 *
							DEGREES(ACOS(COS(RADIANS(".$lat."))
								 * COS(RADIANS(PROFESIONALES.LATITUD))
								 * COS(RADIANS(".$long.") - RADIANS(PROFESIONALES.LONGUITUD))
								 + SIN(RADIANS(".$lat."))
								 * SIN(RADIANS(PROFESIONALES.LATITUD))))>0.0 AND 111.1111 *
							DEGREES(ACOS(COS(RADIANS(".$lat."))
								 * COS(RADIANS(PROFESIONALES.LATITUD))
								 * COS(RADIANS(".$long.") - RADIANS(PROFESIONALES.LONGUITUD))
								 + SIN(RADIANS(".$lat."))
								 * SIN(RADIANS(PROFESIONALES.LATITUD))))<30.0
						ORDER BY
                            DISTANCIA
                        ASC";
		...
		public function buscarServiciosConFiltro($arrayServicios,$provincia,$ciudad){
			$con = new Conexion();
			session_start(); //la idea es que no muestre resultados con la misma id de sesion , pero crashea al habiltiarlo
			$servicios="";
			$arrayServicios=json_decode($arrayServicios);
			...
			$sql="SELECT
					...
					WHERE
						USUARIOS.idUSUARIO = PROFESIONALES.USUARIO_idUSUARIO AND OFICIOS.PROFESIONAL_idPROFESIONAL = PROFESIONALES.idPROFESIONAL AND OFICIOS.SERVICIOS_idSERVICIO = SERVICIOS.idSERVICIO AND(
							".$servicios."
						) AND OFICIOS.HABILITADO=1 ".$idusuario." AND USUARIOS.LOCALIDAD='".$ciudad."' AND USUARIOS.PROVINCIA='".$provincia."' ";
			$query = $con->prepare($sql);
			if($query->execute()){
				$result = $query->fetchAll();
				$datos = array();
